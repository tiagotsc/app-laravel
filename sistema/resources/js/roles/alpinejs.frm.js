    ...
    Alpine.data("managerFrm", () => ({
        ...
        loadData(){ // carrega dados tela
            ...
            fetch(document.getElementById("route_list").value)
                                ...
                                .then(res => {
                                    this.data = res.data;
                                    document.getElementById("name").value = (res.data?.name != undefined)? res.data?.name: '';
                                    this.selectedPermissions = arrayColumn(res.data.permissions, 'name');
                                    this.allPermissions = res.allPermissions;
                                    //loadingHide();
                                    this.loading = false;
        ...
        feedLi(){
            ...
            this.filteredIPermissions.forEach(per => {
                if(per.show){
                    let checked = (this.selectall || (this.selectedPermissions.indexOf(per.name) >= 0 && this.clicked == false) ? 'checked' : '')
                    lis += `<li class="list-group-item"> 
                                <input type="checkbox" id="permission[${per.name}]" class="input_checkbox"
                                name="permission[${per.name}]" value="${per.name}" 
                                ${checked}>
                            <label id="name" for="permission[${per.name}]">${per.name}</label>
                        </li>`;
                }
            })
            document.getElementById("list_permission").innerHTML = lis;
        },
        allChecked(){ // Marca ou desmarca todos os checkboxs
            this.clicked = true; /* marca como true quando a função allChecked é executada */
            this.selectall = !this.selectall;
