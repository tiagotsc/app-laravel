/* function format ( d ) {
    ...
    var table = $('#list').DataTable( {
        ...
        columns: [
            ...
            {
                data: 'user_registration', width: "8%", orderable: false,
                render: function ( data, type, row ) { 
                    return row.user_registration.split(',').join("<br>");
                }
            },
            { 
                data: 'user_id', 
    ...
    $(function() {
        ...
        $('#user_registration').select2({ placeholder: "Selecione..." });
        $( '#user_registration' ).rules( "add", { // Adiciona validação, após a carga da combo
            required: true,
            messages: {
                required: "Selecione, por favor"
            }
        });
        $('#user_registration').on("change", function (e) {
            $(this).valid();
        });
    });
