...
//             // Pegando valores
//             this.valueAll.valueTotal = this.sumColumn(resultRows.data, 'value_total', 'float').toFixed(2);
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data("managerFrm", () => ({
        ...
        correctionEnable: document.getElementById("correction_enable").value, // Informa se campos de correção devem ser exibidos
        correctionShow: true, // Controla exibição dos campos de correção
        data: {
            _token: document.head.querySelector('meta[name=csrf-token]').content
        }, // dados do formulário
        rows: {}, // Armazena a lista de dados
        ...
        async getList(){ // Carrega Lista
            this.correctionShow = (this.correctionEnable == 'true')? true: false;
            this.rows = {}; // Limpa lista
            this.qtdAll = {...objQtd}; // Zera as quantidades, clonando objeto zerado original
            this.valueAll = {...objValue}; // Zera os valores, clonando objeto zerado original
            let url = document.getElementById("route_list").value;
        ...
        */
        ...
        feedsTbody(rows){
            ...
            tbody.innerHTML="";
            rows.forEach(item => {
                let title = (Boolean(item.name))? item.name: '';
                let first = `${item.batch_date.substr(8,2)}/${item.batch_date.substr(5,2)}/${item.batch_date.substr(2,2)}`;

                let fieldsCorrection = '';
                if(this.correctionEnable == "true"){ // Deve exibir campos de correção
                    fieldsCorrection = `<td>${this.removeNull(item.count_correction)}</td>
            ...
            if(rows.length == 0){
                let colspan = (this.correctionEnable == 'true')? 12: 10;
                tbody.innerHTML = `<tr><td colspan="${colspan}" class="text-center"><b>Nenhum registro encontrado</b></td></tr>`;
            }
        },
        feedsTfoot(qtdAll, valueAll){
            ...
            let fieldsCorrection = '';
            if(this.correctionEnable == "true"){ // Deve exibir campos de correção
                fieldsCorrection = `<td>${qtdAll.correction}</td>
            ...
            tr = `<tr class="table-secondary">
                            ...
                            <td>${qtdAll.total}</td>
                            <td>${this.moneyBR(valueAll.valueTotal).substr(3)}</td>
